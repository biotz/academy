...
You can model that use case as a "Collection of identical items", whose child data type is an "Object". The key names of that "Object" would be `"timestamp"`, `"temperature"` and `"humidity"`, with suitable data types for their values (e.g., "[Unix timestamp RFC-3339](#unix-timestamp-rfc-3339), "[Decimal](#decimal)" and "[Integer](#integer)").
